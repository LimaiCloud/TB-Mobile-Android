...
//    private List<ContactSortBean> mDatas;
    private Paint mPaint;
    private Rect mBounds;//用于存放测量文字Rect

    private int mTitleHeight;//title的高
    private static int COLOR_TITLE_BG = Color.parseColor("#FFFFFF");
    private static int COLOR_TITLE_FONT = Color.parseColor("#3783d2");
    private static int mTitleFontSize;//title字体大小


//    public TitleItemDecoration(Context context, List<ContactSortBean> datas) {
//        super();
//        mDatas = datas;
//        mPaint = new Paint();
//        mBounds = new Rect();
...
//    }
    public TitleItemDecoration(Context context, List<UsersBean.DataBean> datas) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
...
/*
        ...
        c.drawText(mDatas.get(position).getSortLetters(), 45, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        String tag = mDatas.get(pos).getSortLetters();
